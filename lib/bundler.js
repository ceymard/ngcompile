	...
	*/
	bundle: (function (bundle) {
		...
		var simplified = '(function() {\n' +
			// Prefixing the name of window with NG_DEFER_BOOTSTRAP, which
			// is to ensure that angular doesn't try to run at the beginning,
			// to give us the time to load the modules asynchronously.
		...
		files.each(function (f) {
			scripts.push('\'' + bundle.base + f.replace(bundle.cwd, '') + '\'');
			full += '/* ' + f + ' */\n' + fs.readFileSync(f, 'utf-8') + '\n';
		});

		simplified += scripts.join(', ');
		simplified += '];\n\n' +
